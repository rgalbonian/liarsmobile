...
export class RequestModalPage {
 ...
 public defaultDate : string;
  ...
  constructor(public actionSheetController: ActionSheetController, public navCtrl: NavController, public navParams: NavParams, private view: ViewController, public modalCtrl: ModalController, private alertCtrl: AlertController, public toastController: ToastController) {
       ...
       var orRequest = this.navParams.get('request')
       this.originalRequest = orRequest;
...
$("button.btn-2").css("display", "none");
  ...
  ionViewDidLoad() {
    ...
    if (n == "notify" || this.request.status == "ready" || this.request.status == "released" || this.request.status == "complete" || this.request.status == "preparing" || this.request.status == "defective" ){
  ...
  cancelRequestCheck(){
  	if (this.request.status == "pending" || this.request.status == "declined" || this.request.status == "complete"){
	  	this.presentCancelConfirm();
  	}else{
  		this.presentAlertPendingOnly2();
  		//console.log("noooooo")
  	}
  }
  cancelRequest(){
  ...
  saveEdit(){
    var allInputs =  $(':input[type="number"]');
    console.log(allInputs)
    for (var i = allInputs.length - 1; i >= 0; i--) {
      var color = $(allInputs[i]).css("border-bottom-color");
      console.log(color)
      if (color == "rgb(255, 0, 0)"){

        this.presentInvalidValues();
        return false;
      }
    }
  	console.log(this.request);
...
console.log(date);
	...
	presentAlertPendingOnly() {
	  let alert = this.alertCtrl.create({
	    ...
	    subTitle: 'Error! You can only edit pending requests.',
	    buttons: ['Dismiss']
	  });
	  alert.present();
	}

  presentInvalidValues() {
    let alert = this.alertCtrl.create({
      title: 'Invalid values!',
      subTitle: 'Error! Enter valid input.',
      buttons: ['Dismiss']
    });
    alert.present();
  }

  presentAlertPendingOnly2() {
    let alert = this.alertCtrl.create({
      title: 'Request in Process!',
      subTitle: 'Error! You can only delete pending, completed, or declined requests.',
      buttons: ['Dismiss']
    });
    alert.present();
  }
...
$("input[name='quantity']").on('keypress keydown keyup',function(){
 var $regexunit=/^[0-9]+$/;
...
$("input[name='amount']").on('keypress keydown keyup',function(){
  var $regexamount = /^\d*\.?\d*$/;
  var amount = $(this).val();
