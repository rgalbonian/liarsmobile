 ...
 request = {} as Request;
  ...
  console.log(this.request)
  	...
  	histDB.orderByChild("historyID").equalTo(this.request.requestID).on('value', hist => {
                ...
                console.log(hist.val().actions)
                hist.forEach( itemSnap => {
                		var myactions = itemSnap.val().actions;
                		console.log(myactions);
                		if (myactions.length == 1){
                			this.history = myactions
                		}else{
                			//this.history = [];
                			for (var i in myactions){
                				console.log(myactions[i])
                				this.history.push(myactions[i])
                			}
                			
                			//this.history = myactions.actions;
                		}
